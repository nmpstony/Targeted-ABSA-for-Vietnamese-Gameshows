...
1. **Tổng quan (Overview):** Biểu đồ phân phối tự động vẽ bằng Chart.js cho các khía cạnh Aspect (PERFORMANCE, PERSONALITY, SONG, TEAMWORK, VISUAL, SHOW_FORMAT), cực tính cảm xúc (positive, negative, neutral), và nguồn dữ liệu (Rap Việt, Anh Trai Say Hi). Hiển thị chỉ số Cohen's Kappa score được tính động.
2. **Live Playground:**
   - **Chạy đơn lẻ (Single Model):** Chạy suy luận trực tiếp trên câu nhập tự do hoặc chọn câu mẫu có sẵn.
   - **Chạy so sánh (Comparative Mode):** So sánh side-by-side kết quả của cả 4 mô hình (`xlm-roberta-large`, `vibert`, `qwen`, `llama`) cùng lúc.
   - **Tối ưu hóa tài nguyên:** Các mô hình (ViBERT, XLM-RoBERTa-Large, Qwen và Llama) được chạy tuần tự và **tự động giải phóng VRAM ngay sau khi chạy xong** để tránh quá tải bộ nhớ máy tính.
   - **Highlight trực quan:** Bôi màu riêng biệt đối với **Đối tượng (Target)** để dễ dàng quan sát (ẩn Opinion Span đối với dự đoán mô hình phân loại).
...
*Lưu ý: Các checkpoint mô hình lớn và phân loại tinh chỉnh (như ViBERT-Base, XLM-RoBERTa-Large, Qwen2.5-3B-Instruct, Llama-3.2-3B-Instruct) cần được đặt tương ứng trong các thư mục con của `Model/` để server nạp và chạy suy luận.*
